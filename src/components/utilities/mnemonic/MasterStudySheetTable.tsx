...
export const MasterStudySheetTable: React.FC<MasterStudySheetTableProps> = ({ items }) => {
  ...
  return (
    <div className="space-y-6">
      {/* Top Banner Info */}
      <div className="p-3.5 sm:p-4 rounded-xl bg-indigo-50/70 dark:bg-indigo-950/40 border border-indigo-200 dark:border-indigo-800 flex flex-wrap items-center justify-between gap-3 text-xs">
        ...
        <span className="px-2.5 py-1 rounded-lg bg-indigo-100 dark:bg-indigo-900/80 text-indigo-800 dark:text-indigo-200 font-bold">
      ...
      <div className="space-y-4 sm:space-y-6">
        {items.map((item, idx) => {
          ...
          return (
            <div
              key={item.id || idx}
              className="rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition-all overflow-hidden"
            >
              {/* Row Header */}
              <div className="px-4 sm:px-5 py-2.5 bg-slate-50 dark:bg-slate-950/80 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between gap-2">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 h-8 px-3 rounded-lg bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 text-xs font-bold text-slate-700 dark:text-slate-300 transition-colors cursor-pointer shadow-xs"
                >
                  {copiedId === item.id ? (
                    <>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 sm:gap-6 p-4 sm:p-6 items-center">
                ...
                <div className="lg:col-span-7 space-y-4">
                  ...
                  <div className="p-3.5 rounded-lg bg-amber-100/70 dark:bg-amber-950/40 border border-amber-300 dark:border-amber-700/60 shadow-xs space-y-1">
                    ...
                    <div className="text-sm sm:text-lg font-black text-amber-950 dark:text-amber-100 leading-snug">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {item.breakdown.map((b, bIdx) => (
                        <div
                          key={bIdx}
                          className="flex items-center gap-2 p-2 rounded-lg bg-slate-50 dark:bg-slate-800/80 border border-slate-200/80 dark:border-slate-700"
                        >
                  ...
                  {item.mcq_avoid_confusion_tip && (
                    <div className="flex items-start gap-2 p-2.5 rounded-lg bg-rose-50 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-900/50 text-rose-900 dark:text-rose-200 text-xs">
                ...
                <div className="lg:col-span-5 space-y-3">
                  <div className="relative rounded-xl overflow-hidden border border-amber-300 dark:border-amber-700/60 shadow-md group bg-slate-100 dark:bg-slate-800">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img
                      src={displayImageUrl}
                      alt={item.mnemonic_formula}
                      className="w-full h-52 sm:h-60 object-cover group-hover:scale-105 transition-transform duration-500 cursor-pointer"
                      onClick={() =>
                        setZoomImage({
                          url: displayImageUrl,
                  ...
                  {item.visual_cue.cartoon_dialogue && (
                    <div className="p-2.5 rounded-lg bg-[#FDFBF7] dark:bg-slate-800/90 border border-amber-200 dark:border-slate-700 text-xs text-amber-900 dark:text-amber-200 font-semibold italic text-center">
                      💬 "{item.visual_cue.cartoon_dialogue}"
                    </div>
                  )}
                </div>
      ...
      {zoomImage && (
        ...
        >
          <div
            ...
            className="relative max-w-3xl w-full bg-slate-900 rounded-2xl overflow-hidden border border-slate-700 shadow-2xl p-4 space-y-3"
          >
