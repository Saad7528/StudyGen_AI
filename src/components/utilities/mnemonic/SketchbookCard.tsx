...
export const SketchbookCard: React.FC<SketchbookCardProps> = ({
  ...
  return (
    <div className="relative rounded-3xl bg-[#FDFBF7] dark:bg-slate-900 border-2 border-[#E9DFCB] dark:border-slate-800 shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden group">
      {/* Top Paper Header Bar */}
      <div className="px-5 py-3.5 bg-[#F7F2E6] dark:bg-slate-950/80 border-b border-[#E8DFC8] dark:border-slate-800 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button
            ...
            className="p-1.5 rounded-xl bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors cursor-pointer"
          >
      ...
      <div className="p-5 sm:p-6 space-y-5">
        ...
        <div className="relative p-4 sm:p-5 rounded-2xl bg-amber-100/70 dark:bg-amber-950/40 border-2 border-amber-300 dark:border-amber-700/60 shadow-xs">
          ...
          <div className="text-lg sm:text-xl font-extrabold text-amber-950 dark:text-amber-100 tracking-wide leading-relaxed">
        ...
        <div>
          <div className="text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-2.5 flex items-center gap-1.5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2.5">
            {item.breakdown.map((b, bIdx) => (
              <div
                key={bIdx}
                className="flex items-center gap-2.5 p-2.5 rounded-xl bg-white dark:bg-slate-800/80 border border-slate-200/80 dark:border-slate-700 shadow-xs"
              >
                <div className="min-w-8 px-2 h-8 rounded-lg bg-indigo-600 text-white font-extrabold text-xs flex items-center justify-center shrink-0 shadow-xs text-center">
                  {b.code}
                </div>
        ...
        {item.mcq_avoid_confusion_tip && (
          <div className="flex items-start gap-2.5 p-3 rounded-2xl bg-rose-50 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-900/50 text-rose-900 dark:text-rose-200 text-xs">
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            {onSelectForPresentation && (
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800 text-xs font-bold hover:bg-indigo-100 dark:hover:bg-indigo-900 transition-colors cursor-pointer"
              >
