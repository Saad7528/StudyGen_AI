  ...
  return (
    <div className="w-full rounded-2xl p-6 sm:p-8 bg-gradient-to-b from-indigo-50/70 via-white/70 to-purple-50/50 dark:from-slate-900/90 dark:via-slate-900/60 dark:to-indigo-950/40 border border-slate-200/80 dark:border-slate-800 backdrop-blur-xl shadow-xl overflow-hidden relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
        ...
        {/* =========================================================
            LEFT / TOP NAVIGATION: Responsive Step List / Timeline
           ========================================================= */}
        {/* Mobile/Tablet Compact Segmented Bar (< lg) */}
        <div className="lg:hidden col-span-1 space-y-3">
          <div className="grid grid-cols-3 gap-1.5 p-1 rounded-xl bg-slate-100/90 dark:bg-slate-800/90 border border-slate-200/80 dark:border-slate-700/80">
            {steps.map((item, index) => {
              ...
              return (
                <button
                  ...
                  className={`py-2 px-1 rounded-lg text-[11px] font-bold flex flex-col sm:flex-row items-center justify-center gap-1 transition-all cursor-pointer ${
                    isActive
          ...
          <div className="flex items-center justify-between px-1 text-xs text-slate-500">
            <button
              ...
              className="px-2.5 py-1 rounded-md bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:text-indigo-600 transition flex items-center gap-1 font-bold text-[11px] cursor-pointer"
            >
              {isPlaying ? <Pause className="w-3 h-3 text-amber-500" /> : <Play className="w-3 h-3 text-indigo-500" />}
              <span>{isPlaying ? 'অটো-সিমুলেশন চলছে' : 'প্লে করুন'}</span>
            </button>
            <span className="text-[11px] text-indigo-600 dark:text-indigo-400 font-bold">
              ধাপ {activeStep + 1} / ৩: {steps[activeStep].title}
            </span>
          </div>
        </div>

        {/* Desktop Step Cards (lg+) */}
          ...
          {steps.map((item, index) => {
            ...
            return (
              <div
                ...
                className={`group p-4 rounded-xl transition-all duration-300 border cursor-pointer relative overflow-hidden ${
                  isActive
